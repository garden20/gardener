...
exports.local_name = function(ddoc_url, module_name) {
    ...
    if (module_name && !exports.is_tgz(module_name)) {
        var nv = module_name.split("@"),
            name = nv[0];
    ...
    request({url: ddoc_url, json: true}, function (error, response, body) {
      if (!error && response.statusCode == 200) {
        ...
        if (body._attachments && body._attachments[body.node_module] && body._attachments[body.node_module].digest) {
            digest = body._attachments[body.node_module].digest;
        }
        ...
        if (!error || (response && response.statusCode == 200)) {
            ...
            body.rows.forEach(function(row) {
                if (row.doc.node_module) {
                    ...
                    logger.info(row.id + ' on db ' + db_name + ' has a module');
                    ddocs.push({
                        ddoc_id: row.id,
                        db_name: db_name,
