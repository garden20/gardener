...
function poll(couch_url, base_url, includes, excludes, dashboard, callback) {
    logger.info('polling couch for db names.');
    async.auto({
        ...
        callback(err, data.tree);
    });
}


function polld(couch_url, base_url, includes, excludes, dashboard) {
    ...
    async.auto({
        ...
        remove: ['remove_poll', function(cb, data){
            data.remove_poll.forEach(function(module){
                process_manager.uninstall(module.local_name, module, cb);
            });
        }]
    },function(err, data){
        if (err) {
...
function remove_poll(modules, callback) {
    ...
    async.each(modules, function(module, cb){
        ...
        request.head(url, function(err, resp){
            if (resp.statusCode === 404) to_remove.push(module);
            cb();
        });
    }, function(err){
