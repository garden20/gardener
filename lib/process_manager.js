...
exports.start = function(couch_root, callback) {
...
function _fill_missing_install_details(ddoc_url, other, callback) {
    ...
    module_name(ddoc_url, other, data, function(err, module_name){
        ...
        async.parallel([
            ...
            function is_local_package(cb) {
                var suffix = '.tgz';
                data.is_local_package =  (data.module_name.indexOf(suffix, data.module_name.length - suffix.length) === -1);
                cb();
            },
            ...
            function local_name(cb)  {
                ...
                else data.local_name = utils.local_name(ddoc_url);
                cb();
            },
            function db_name(cb) {
                if (other.db_name) {
