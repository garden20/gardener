...
exports.uninstall_db = uninstall_db;
exports.uninstall_ddoc = uninstall_ddoc;
...
function install(details, callback) {
...
function list(/* optional */ db, callback) {
    var localnames = _.keys(running_processes);

    if (_.isFunction(db)){
        async.map(localnames, load_cache_details, db);
    } else {
        // a db to filter on has ben specified
        async.map(localnames, load_cache_details, function(err, results){
            if (err) return callback(err);

            var filtered_to_db = _.filter(results, function(cache_detail) {
                return (cache_detail.db_name === db);
            });
            callback(null, filtered_to_db);
...
function uninstall_ddoc(ddoc_url, callback) {

    var prefix = utils.scrub_auth_from_url(ddoc_url);

    var localnames = _.keys(running_processes);
    async.eachLimit(localnames, 1, function(local_name, cb){
        var real = utils.fromBase64(local_name);

        if (real.indexOf(prefix) === 0) {
            uninstall(local_name, null, cb);
        } else cb();
    }, callback);
}

function uninstall_db(db, callback) {
    list(db, function(err, cache_details) {
        if (err) return callback('Could not list db ' + db + ' processes to end');
        if (!cache_details || cache_details.length === 0 ) callback();

        cache_details.forEach(function(details){
            processes.uninstall(details.local_name, details, callback);
        });
    });
}


function uninstall(local_name, details, callback) {
    if (!details) {
    ...
    is_installed(details, function(err, installed){
        ...
        async.auto({
            ...
            install_package: ['stop_module', function(cb) {
                ...
                npm_manager.install(package_url, working_dir, cb);
            }],
...
function _uninstall(err, details, callback) {
    ...
    async.auto({
        stop: function(cb){
            progress('Stopping module' + details.module_name, 15, details);
            logger.info('Stopping module ' + details.module_name);
            stop(details.local_name, cb);
        },
        deleting: ['stop', function(cb){
            var working_dir = module_working_dir(details.local_name);
            progress('Removing module' + details.module_name, 80, details);
            logger.info('Removing module ' + details.module_name);
        ...
        clean: ['deleting', function(cb){
            ...
            logger.info('Uninstalled ' + details.module_name);
            cb(null);
        }]
    }, callback);
