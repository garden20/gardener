...
proto.watch_couch = function(callback) {
    ...
    gardener._poll_interval = setInterval(function(){
        poll(gardener._couch_root_url, gardener.base_url, include, exclude);
    }, time);

    poll(gardener._couch_root_url, gardener.base_url, include, exclude, true);

    if (dashboard_db) {
        dashboard.follow(gardener._couch_root_url, dashboard_db);
    }
